...
| [automation.md](automation.md) | the automation runtime: `ctx.automation` durable record set, trigger vocabulary, due dispatch, and Session create/resume runs |
| [memory.md](memory.md) | the memory seam: `ctx.memory` single-provider registry, entry vocabulary, the shared error contract, and the model-facing tools and index section |

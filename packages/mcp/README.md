...
The `mcp/` group connects the harness to the Model Context Protocol (MCP) ecosystem in both directions: `mcp-client` attaches external tool servers so the model can call their tools as native tools, and `mcp-server` serves the harness's own registered tools to external MCP clients over stdio. Each server is one configuration entry; nothing ships enabled, so you opt in per server. Only the Tools capability is bridged: MCP resources and prompts are not supported. This page maps the group; the package README owns the per-package contract.
...
| [`mcp-server/`](mcp-server/README.md) | Serve the harness's registered tools to external MCP clients over stdio |

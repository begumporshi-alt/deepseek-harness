...
随附 Web 的 `standard`、`ptc` 与 `cordis` preset 包含[显式文件交付](../../client/ui-deliverables/README.zh.md#explicit-deliveries)。`minimal` preset 保留固定的双工具训练配置。`research` preset 组合出只读优先的分析型 Agent，其委派名册是类型化的：`researcher` 与 `reviewer` 工具在配置中固定子代理的 persona 与只读工具白名单，父代理只需指明任务，无需每次重新描述角色。

...
The shipped Web `standard`, `ptc`, and `cordis` presets include [explicit file delivery](../../client/ui-deliverables/README.md#explicit-deliveries). The `minimal` preset keeps its fixed two-tool training configuration. The `research` preset composes a read-first analysis agent whose delegation roster is typed: its `researcher` and `reviewer` tools fix each child's persona and read-only tool allow-list in configuration, so the parent names the work instead of re-specifying the role.

/** The MCP app command provider and stdin shutdown binding. */

import { EventEmitter } from 'node:events'
import { Context } from '@deepseek-ai/cordis'
import { afterEach, describe, expect, it } from 'vitest'
import { internals, provideCmdline } from '@deepseek-ai/dsh-cmdline'
import { MCP_APP_STARTUP_SERVICE, apply } from '../src/index.ts'
...
describe('MCP app startup', () => {
  ...
  it('prints app help without publishing readiness or binding stdin', () => {
    ...
    expect(out()).toContain('dsh --profile mcp')
    expect(ctx.get(MCP_APP_STARTUP_SERVICE)).toBeUndefined()
    expect(exits).toEqual([0])
    stdin.end()
    expect(exits).toEqual([0])
  })

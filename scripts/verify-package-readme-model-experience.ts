...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/memory/memory': { kind: 'indirect', reason: 'The seam delegates all model rendering to dsh-tool-memory; it registers no prompt, schema, or section of its own.' },
  'packages/automation/automation': { kind: 'indirect', reason: 'The runtime delegates all model rendering to dsh-tool-automation; it registers no prompt, schema, or section of its own.' },
  'packages/mcp/mcp-server': { kind: 'indirect', reason: 'The bridge serves tools to external MCP clients; it registers no prompt, schema, or section of its own in the harness.' },
  'packages/bundle/mcp-app': { kind: 'indirect', reason: 'The bundle is composition wiring around dsh-mcp-server; the bridge it mounts owns every model-facing effect.' },
  'packages/memory/memory-local': { kind: 'none', reason: 'The provider stores entries outside any model request; dsh-tool-memory owns every model-facing effect.' },
